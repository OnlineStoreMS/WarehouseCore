...
async function load() {
  loading.value = true
  try {
    const res = await api.listOtherIn({
      page: page.value,
...
function openCreate() {
  form.value = {
    ...
    reason: 'opening',
    remark: '',
    items: [{ invSkuId: undefined, qty: 1, cost: 0 }],
  }
  visible.value = true
}

function addItem() {
  form.value.items.push({ invSkuId: undefined, qty: 1, cost: 0 })
}
...
async function showDetail(row: any) {
  try {
    detail.value = await api.getOtherIn(row.id)
...
async function post(row: any) {
  await ElMessageBox.confirm('确认仓库审核过账入库？', '提示')
  await api.postOtherIn(row.id)
...
async function cancel(row: any) {
  ...
  await api.cancelOtherIn(row.id)
  ...
  <div class="page">
    <el-card v-loading="loading">
      <template #header>
        <div class="hdr">
          <span>其它入库单</span>
          <el-button type="primary" :icon="Plus" @click="openCreate">新增其它入库单</el-button>
        </div>
      </template>
      ...
      <div class="toolbar">
        <el-select v-model="warehouseId" clearable filterable placeholder="入库仓库" style="width: 180px" @change="search">
          <el-option v-for="w in warehouses" :key="w.id" :label="w.name" :value="w.id" />
        </el-select>
        <el-select v-model="reason" clearable placeholder="入库类别" style="width: 140px" @change="search">
          <el-option v-for="(label, key) in reasonMap" :key="key" :label="label" :value="key" />
        </el-select>
        <el-input v-model="keyword" clearable placeholder="入库单号" style="width: 180px" @keyup.enter="search" />
        <el-button type="primary" @click="search">查询</el-button>
      </div>

      <el-table :data="list" border stripe>
        <el-table-column prop="docNo" label="入库单号" width="160" fixed show-overflow-tooltip />
        <el-table-column label="入库仓库" width="140" show-overflow-tooltip>
          <template #default="{ row }">{{ row.warehouseName || '-' }}</template>
        </el-table-column>
        <el-table-column label="入库类别" width="100">
        ...
        <el-table-column prop="remark" label="入库备注" min-width="140" show-overflow-tooltip />
        <el-table-column label="入库单状态" width="100">
    ...
    <el-dialog v-model="visible" title="新增其它入库单" width="780px">
      <el-form :model="form" label-width="90px">
        ...
        <el-form-item label="入库备注"><el-input v-model="form.remark" /></el-form-item>
        <el-form-item label="明细">
          <el-table :data="form.items" border size="small" style="width: 100%">
            ...
            <el-table-column label="入库单价" width="130">
              <template #default="{ row }">
                <el-input-number v-model="row.cost" :min="0" :precision="2" :controls="false" style="width: 110px" />
              </template>
            </el-table-column>
    ...
    <el-drawer v-model="detailVisible" title="入库单商品明细" size="720px">
      <template v-if="detail">
        <el-descriptions :column="2" border class="mb">
          <el-descriptions-item label="入库单号">{{ detail.docNo }}</el-descriptions-item>
          <el-descriptions-item label="入库仓库">{{ detail.warehouseName || '-' }}</el-descriptions-item>
          <el-descriptions-item label="入库类别">{{ reasonMap[detail.reason] || detail.reason }}</el-descriptions-item>
        ...
        <el-table :data="detail.items || []" border stripe size="small" max-height="480">
          ...
          <el-table-column prop="cost" label="入库单价" width="90" align="right" />
