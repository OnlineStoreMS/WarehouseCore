...
async function load() {
  loading.value = true
  try {
    const res = await api.listTransfers({ page: page.value, pageSize: pageSize.value })
    list.value = res.list
  ...
  <div class="page">
    <el-card v-loading="loading">
      ...
      <el-table :data="list" border stripe>
        ...
        <el-table-column label="调出仓" width="130">
          <template #default="{ row }">{{ whName(row.fromWarehouseId) }}</template>
        </el-table-column>
        <el-table-column label="调入仓" width="130">
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 'draft' || row.status === 'in_transit'" link type="danger" @click="cancel(row)">取消</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
    ...
    <el-dialog v-model="visible" title="新建调拨单" width="640px">
